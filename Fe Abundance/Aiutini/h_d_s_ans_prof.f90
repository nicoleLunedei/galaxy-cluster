...
rho0nfw=7.35d-26
rs=435.7*cmkpc
rho0=1.6e-25  !! FB: changed    !!8d-26 	      !!central density/initial condition
...
mvir=1.3e15*msol
mbgc=1.d12*msol
ahern=12.*cmkpc/(1.+2.**(0.5))   !! FB: 12 kpc instead of 10
zfesol= 1.8d-3
...
!!gravitational energy    !! FB: energy???
grvnfw(1)=0.     
!!!!Cycle on the Original domain!!
do j=2,jmax
...
!!   lnd(j)=lnd(j-1)-(gg*(mu*mp)*(rr(j)-rr(j-1))/(boltz*T(j))+ lnT(j)-lnT(j-1))
   lnd(j)=lnd(j-1) - gg*(mu*mp)*(rr(j)-rr(j-1))/&
          (boltz*0.5*(T(j)+T(j-1)))- lnT(j) + lnT(j-1)
!! FB: 2 problems here: T(j)--> 0.5*(T(j)+T(j-1))  and   -lnT(j)+lnT(j-1)
end do 
do j=1,jmax
   rho(j)=exp(lnd(j))  
enddo
...
open(21,file='diff.dat',status='unknown')  !! FB: "my" file
...
time = time* years*1.d9      !! FB: now the initial time is given in Gyr
dt = 0.4 * (r(5) - r(4))**2 / (2. * D)

  print*,'time, tnow (Gyr) =',real(time/3.156d16),real(t_now/3.156d16)
  print*,'dt (yr) = ',real(dt/years)
...
!!read(*,*) coeff   !! FB: watch this, always constant source 

coeff=0    !! FB

!! do n=1, int(n_cycle)         !! FB changed the do loop

do while (time.le.t_now)
...
!!      rho_hern(j) = (mbgc * ahern)/ (2 * 3.14 * r(j) * ((r(j) + ahern)**3)) 
      rho_hern(j)=mbgc/(2.*pi)*(ahern/rr(j))/(ahern+rr(j))**3 !! FB: changed!
      if (coeff==1) then
...
do j=1, jmax
   ...
   if (coeff==0) then
      write (40, 1005) r(j)/cmkpc, rho_fe_obs(j), rho_fe(j), zfe_obs(j)/zfesol, zfe_ex(j)/zfesol, z_fe(j)/zfesol
          write(21,3000)rr(j)/cmkpc,z_fe(j)/zfesol,rho(j)
   end if
end do
close(20)
close(30)
