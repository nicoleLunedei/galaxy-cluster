!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!! Codice con il coefficiente di diffusione variabile
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
real*8, dimension(jmax) :: r,rr,vol,mnfw,&
        ...
        ne_reb, rho_reb, T_reb, D, zfe_obs,& !!!!Variables for diffusion program
...
real*8 :: msol,mu,mp,rmin,rmax,mvir,rvir,mbgc,ahern,fb,rkpc,&
          zfesol,zfe_ground,rho_jp1,rho_j, v_t, l_t, rs, rscala_D,&
          t_final, time, n_cycle, D_max, dt
!constants
...
!Comparison between our profiles, of density and temperature, and the Rebusco profiles!
 ...
 !!Cycle on the Original domain!!
  ...
  zfesol= 1.8d-3
  zfe_ground = 0.4 * zfesol     !!Z background
  v_t = 200d6                   !!turbolence velocity for D const
  l_t = 20 * cmkpc              !!lenght scale for D const
  KeV = 1.16d7
  rscala_D= 30.*cmkpc           !!scalar radius for D profile
  !D = 0.333333333 * v_t * l_t  !!Diffusion coefficient from Rebusco
  !D=0.11*260d5*15*cmkpc        !!Diffusion coefficient from prof
...
do j = 1, jmax
 ...
 !DIFFUSION CASE!
 !!Diffusion coefficient radial profile
   D(j)=D(j)-0.6*D(j)*exp(-(r(j)/rscala_D)**2)
...
!!!!!!!!!TIME CYCLE!!!!!!!!!!!!!!!!!!!!***************************************************************************
t_final = 1.e9 * years        !!total time 
n_cycle=t_final/dt
D_max= maxval(D)              !!we're gonna use this to calculate dt=C*min{dx^2/2D}
!!Calculating the time step
dt = 0.4 * (r(5) - r(4))**2 / (2. * D_max)   
Print*, "t_final:", t_final
Print*, "Coefficiente di diffusione ", D_max
time = 0.
...
!!!!!!!!!!!!!!Computing the grad Zfe!!!!!!!!!!!!!!
 ...
 !!!Medium values of the gas density in each shell!!!
  ...
  !! So now we have the gas density on the grid r(j), and note that they are numeric values not vectors
      ...
      rho_fe(j) = rho_fe(j) &
            + (dt/1.4)*(r(j+1)**2*D(j+1)*rho_jp1*gradz_fe(j+1) &
            -r(j)**2*D(j)*rho_j*gradz_fe(j))   &
