...
open(10,file='grid_fediff.dat',status='unknown')
do j=1,jmax
...
!
  ...
  sinv=1./sbill
!
open(20,file='temperature_fediff.dat',status='unknown')
 do j=1,jmax
    rkpc=rr(j)/cmkpc
    roverr0 = rkpc/r0bill
...
open(20,file='density_fediff.dat',status='unknown')
do j=1,jmax
   write(20,1101)rr(j)/cmkpc,rho(j),rhonfw(j),rhost(j)
enddo
close(20)

open(20,file='mgas_fediff.dat',status='unknown')
...
open(20,file='barfrac_fediff.dat')
do j=2,jmax-1
...
open(20,file='initial_fediff.dat',status='unknown')
do j=1,jmax
...
!!  set the diffusion coefficient kappa = C*v*l (for now constant)
 ...
 open(20,file='kappa_fediff.dat',status='unknown')
...
!!      print*,'TIME (Gyr) = ',time/3.156e16
      ...
      open(21,file='diff_fediff.dat',status='unknown')
      do j=2,jmax
